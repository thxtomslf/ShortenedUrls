	...
	getSrcTestTable := []FirstTestGroupRequest{
		{
			"GetTest1",
			"GET",
			"http://:8080/" + receivedUrlsList[0]["short_url"],
		...
		},
		{
			"PostTest",
			"POST",
			"http://:8080/" + receivedUrlsList[1]["short_url"],
			"",
			http.StatusNotFound,
			PostTest,
		},
		...
		},
		{
			"GetTest3",
			"GET",
			"http://:8080/" + receivedUrlsList[3]["short_url"],
